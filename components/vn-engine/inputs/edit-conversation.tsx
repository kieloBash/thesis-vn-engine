...
import {
  ...
  Conversation,
  isConversation,
} from "@/types/vn-engine/main-types";
import React, {
  FormEvent,
  KeyboardEvent,
  useEffect,
...
const EditConversationInput = () => {
  ...
  useEffect(() => {
    if (
      selectedCommand &&
      selectedSlide &&
      isConversation(selectedSlide.dialogue)
    ) {
      setSpeaker(selectedSlide.dialogue.speaker);
      setAltName(selectedSlide.dialogue?.altName || "");
      setcurrDialogue(selectedSlide.dialogue.line);
      setxPos(selectedSlide.dialogue?.startXPos || 0.5);
      setisHidden(selectedSlide.dialogue?.isHidden || false);
    }
  ...
  function handleSubmit(e: FormEvent) {
    e.preventDefault();
    if (!speaker || currDialogue === "" || !selectedCommand) return null;
    ...
    newSlides[selectedIdx] = newSlide;
...
export default EditConversationInput;
